...
// Soo qaado habeynta hadda, haddii aan la dejin ka dhig 0.
...
// Xakamee gudbinta dalabka
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $pdo = get_db_connection();

    try {
        // Start Transaction
        // Bilow macaamilka (transaction)
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <?php if ($success): ?>
            <div class="bg-white rounded-[3rem] p-20 text-center shadow-2xl border border-slate-100">
                ...
                <p class="text-slate-500 mb-10 text-lg">Thank you for your order #<?php echo $order_id; ?>. We're currently
                    preparing your delicious meal!</p>
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
                    <a href="./"
                        class="bg-orange-600 text-white px-10 py-4 rounded-full font-bold shadow-xl shadow-orange-200">Track
                        Order</a>
                    <a href="./products" class="bg-slate-100 text-slate-600 px-10 py-4 rounded-full font-bold">Back to
                        Menu</a>
                </div>
            </div>
        <?php else: ?>
                    ...
                    <form action="" method="POST" class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="col-span-2">
                                ...
                                <textarea required
                                    class="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-orange-500 focus:outline-none transition h-32"
                                    placeholder="Street name, Building, Apartment..."></textarea>
                            </div>
                            <div>
                        ...
                        <div class="pt-10">
                            <h3 class="text-2xl font-bold text-slate-900 mb-8 border-b border-slate-50 pb-6">Payment Method
                            </h3>
                            <div class="grid grid-cols-2 gap-4">
                                <label class="relative cursor-pointer group">
                                    <input type="radio" name="payment" value="cash" checked class="peer sr-only">
                                    <div
                                        class="p-6 border-2 border-slate-100 rounded-2xl peer-checked:border-orange-500 peer-checked:bg-orange-50 transition">
                                        <i
                                            class="fas fa-money-bill-wave text-2xl mb-2 text-slate-400 group-peer-checked:text-orange-600"></i>
                        ...
                        <div class="pt-10 border-t border-slate-100 mt-10 space-y-2">
                            ...
                            <div class="flex justify-between items-center pt-4 border-t border-slate-50 mt-4">
                                ...
                                <span
                                    class="text-3xl font-black text-slate-900">$<?php echo number_format($total, 2); ?></span>
                            </div>
                            <button type="submit"
                                class="w-full bg-orange-600 text-white py-6 rounded-2xl font-black text-xl hover:bg-orange-700 transition shadow-2xl shadow-orange-200">
                                Place Order Now
                            </button>
                        </div>
                    </form>
